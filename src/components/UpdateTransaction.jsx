...
export default function UpdateTransaction({open , setOpen , transaction}) {
  const { accounts, updateTransaction } = useFinancialStore();
    const now = new Date();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setLoading(true);
            const newAccount = await updateTransaction(transaction.id,formData);
        ...
        <Dialog.Root size={{ base: 'xs', md: 'md' }} open={open} onOpenChange={(e) => setOpen(e.open)} placement={'center'}>
            <Portal>
                ...
                <Dialog.Positioner>
                    <Dialog.Content>
                        <Dialog.Header>
                            <Dialog.Title>Update Transaction</Dialog.Title>
                        </Dialog.Header>
                        <Dialog.Body pb="4">
                            <Stack gap="4">
                                <Field.Root>
                        ...
                        <Dialog.Footer>
                            ...
                            <Button
                                ...
                                colorScheme="blue"
                            >
                                {loading ? <Spinner size={'xs'} /> : 'Update Transaction'}
                            </Button>
                        </Dialog.Footer>
                    </Dialog.Content>
                </Dialog.Positioner>
